...
'''
import glob
#files = glob.glob('/eos/user/a/aradosla/SWAN_projects/Analysis/ADTObsBox_analysis/results_nxcals/*.parquet')
#print(files)
...
fills   = df_fills['HX:FILLN'].dropna().unique()
#fills = ['8496']
#print(fills)
#quit()
#print('Finished with the first part -> now plotting')
for fill_nb in fills: #['8306','8345']:
    tt=pd.DataFrame()
    df_fills_onenb = pd.DataFrame() 
    ...
    for beam_plane in ['B1H', 'B2H', 'B1V', 'B2V']:
      ...
      #try:
        ...
        file = glob.glob(f"{path}/plots_{fill_nb}/{beam_plane}/*.parquet")
        ...
        #  df_fills['HX:FILLN'].iloc[0] = df_fills['HX:FILLN'].dropna().iloc[0]
        #df_fills['HX:FILLN'] = df_fills['HX:FILLN'].ffill(axis=0)
        df_fills_onenb = df_fills[df_fills['HX:FILLN'] == fill_nb]
        #df_fills[df_fills['HX:FILLN'] == fill_nb].index = [pd.Timestamp(df_fills.index[i]) for i in range(len(df_fills))]

        df_fills_onenb.index = [pd.Timestamp(df_fills_onenb.index[i]) for i in range(len(df_fills_onenb))]
        #print(df_fills)
        
        df_bunches = df_fills_onenb[f'LHC.BQM.B{beam_plane[1:2]}:NO_BUNCHES'].dropna().astype(int)
        #df_bunches = df_fills[df_fills['HX:FILLN'] == fill_nb][f'LHC.BQM.B{beam_plane[1:2]}:NO_BUNCHES'].dropna().astype(int)
        bunches = df_bunches.max()

        
        save_to  = f"/eos/user/a/aradosla/SWAN_projects/Analysis/ADTObsBox_analysis/results_nxcals10Hz"
        from pathlib import Path
        Path(save_to).mkdir(parents=True, exist_ok=True)

        #save_to  = f"{save_to}/plots_{fill_nb}/{beam_plane}"
        save_to = f"{save_to}/{beam_plane}"

        from pathlib import Path
        Path(save_to).mkdir(parents=True, exist_ok=True)


        if True:#for counter, ii in enumerate(range(0, 5000,200)):#(range(0, 5000,200)):
            ...
            myf = np.array([10.0])#np.arange(ii-10,ii+n+10, 50)
            k = myf/50.
            ...
            if plot_beam_mode:
                for mode in ["PRERAMP", "RAMP", "FLATTOP", "ADJUST", "STABLE"]:#, "SQUEEZE", "STABLE"]:
                    try:
                        #tt=pd.Timestamp(df_fills[df_fills["HX:BMODE"] == mode][df_fills['HX:FILLN'] == fill_nb].iloc[0].name)
                        
                        tt=pd.Timestamp(df_fills_onenb[df_fills_onenb["HX:BMODE"] == mode].iloc[0].name)
                        #[df_fills['HX:FILLN'] == 'fill_nb']
           ...
           #     plt.plot( (df_int['FREQUENCE_RETENUE(EN Hz)']-50.0)*k[i] + myf[i], df_int['DATE'])
            ...
            fig.tight_layout()
            fig.savefig(f"{save_to}/plot_{fill_nb}_0-25.png")
            plt.close("all")
            #quit()
