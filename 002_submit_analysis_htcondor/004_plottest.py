...
bunch_nb = 5
file_path = '004aread_rte_data/RTE_Frequence_2022_11.txt'
...
for filename in [f"FFT_Fill{fill_nb}_Q7_B1H{bunch_nb}.parquet"]:#,
  ...
  save_to  = f"{path}/plots_{fill}_{beamplane}_{pu}_{bunch_nb}_zoom_rte"
  ...
  for counter, ii in enumerate(range(0, 6000,n)):
      ...
      myf = np.arange(ii,ii+n, 50)
      k = myf/50.

      
      first_HX_BMODE = my_df['HX:BMODE'].unique()[0]
      t1 = pd.Timestamp(my_df[my_df['HX:BMODE'] == first_HX_BMODE].iloc[0].name, tz='UTC')
      last_HX_BMODE = my_df['HX:BMODE'].unique()[-1]
      t2 = pd.Timestamp(my_df[my_df['HX:BMODE'] == last_HX_BMODE].iloc[0].name,tz ='UTC')
  ...
  for counter, ii in enumerate(range(0, 5000, n)):
      ...
      myf = np.arange(11245.5-ii , 11245.5-(ii+n), 50)
      k = myf/50.
