...
path = "/eos/user/a/aradosla/SWAN_projects/Analysis/ADTObsBox_analysis/results/run_2023"
#bunch_nb = 3
bunch_nb = 1
#bunch_nb = 11
#bunch_nb = str(20)
...
for filename in [f"FFT_Fill{fill_nb}_Q7_B1H{bunch_nb}.parquet"]:#,
  ...
  my_df = pd.read_parquet(f'/afs/cern.ch/work/a/aradosla/private/ADTObsBox_analysis/002_submit_analysis_htcondor_live/fills.parquet')
  ...
  for counter, ii in enumerate(range(0, 6000,n)):
      ...
      my_df = my_df[my_df["HX:FILLN"] == 'fill_nb']
      for mode in ["SETUP", "INJPHYS", "RAMP", "FLATTOP", "ADJUST", "SQUEEZE", "STABLE"]:
          try:  
          #for mode in ["SETUP", "INJPHYS", "RAMP", "FLATTOP", "ADJUST", "SQUEEZE", "STABLE"]:
